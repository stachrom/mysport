...
Router.map(function(){
   ...
   this.route('kurse', {
        ...
        data:function (){
            ...
            var in_statement = {$in: [Session.get("filterTag")]};
            var in_statement_level = {$in: [Session.get("filterLevel")]}

            if(Session.get("filterTag") === ""){
                in_statement = { $exists: true };
            }
            if(Session.get("filterLevel") === ""){
                in_statement_level = { $exists: true };
            }

        var data = Kurs.find({
                              "Kursdaten.Stop": {$gte: new Date()},
                              Tag: in_statement,
                              Level: in_statement_level,
                             },{sort: Session.get("kurs_table_sort")}).fetch();
    ...
    this.route('kurs', {
        ...
        waitOn: function () {
               return[
                       Meteor.subscribe("kurse", this.params._id),
                       Meteor.subscribe('users', Session.get("coachId"), "kursleiter")
                     ]
        },
        data: function (){
           Cart.remove({});
           ...
           if (data){
              Session.set("coachId", data.Coach);
              if(data && data.Kursdaten && data.Kursdaten.Daten){
                 data.Kursdaten.Daten = _.sortBy(data.Kursdaten.Daten, 'date');
              }
           }
           return data;
        },
    ...
    this.route('admin', {
        ...
        data:function (){
           ...
           return{
                 new_bookings: Kursanmeldungen.find({"Rsvp": "yes"}),
                 exported: Kursanmeldungen.find({"Rsvp": "exported"}),
                 rechnung: Rechnung.find({})
           };
        }
    });
    this.route('coach', {
